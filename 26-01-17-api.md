...
| is_voted | bool | Whether the current user voted for this option |
| voters | array | List of users who voted for this option (id, name) |

**Poll Fields:**
| Field | Type | Description |
|-------|------|-------------|
| total_voters | int | Number of unique users who voted |
| total_votes | int | Total number of votes (may differ from total_voters for multiselect) |
| can_change_status | bool | Whether current user can open/close the poll |
...
Get poll details with all voting information.

**Headers:** `Authorization: Bearer <token>`

**Response:** Poll object (same format as in list response).

**Access Control:** Group members can access group polls. Event participants (joined/maybe) can access event polls.
...
**Access Control:** The following users can change poll status:
- Poll creator
- Group owner
- Group admin
- Event owner (if event poll)
- Event admin (if event poll)
...
| participation | string | all | Participation filter: `joined`, `maybe`, `declined`, `unresponded`, `all` or comma-separated like `joined,maybe`. `unresponded` matches events where user hasn't set their status |
...
```json
{
  "content_type": "message|event|poll|user",
  "content_id": 123,
  "reason": "spam|harassment|hate_speech|illegal|violence|inappropriate|other",
  "comment": "Optional additional details about the report"
}
```
...
| Type | Description |
|------|-------------|
| `message` | Report a chat message (group or event chat) |
| `event` | Report an event (title/description) |
| `poll` | Report a poll (question/content) |
...
| `other` | Other reason (use comment to explain) |
...
|-------------|------------|-------------|
| 404 | content_not_found | The reported content does not exist |
| 403 | no_access | User does not have access to this content |
| 409 | already_reported | User has already reported this content |
| 409 | cannot_report_own_content | Cannot report your own content |

---

### GET /api/v1/reports/{id}

Get details of a specific report. Only accessible by the reporter or group admins.
...
```json
{
  ...
  "content_id": 123,
  "reason": "spam",
  "comment": "This is spam advertising",
...
**Note:** Returns `404 Not Found` if the report doesn't exist or the user doesn't have access.
...
## Changelog from 2026-01-24

### Event Participation Filter Enhancements
- Added `unresponded` option to `participation` filter - matches events where user hasn't set their status yet
- Participation filter now supports comma-separated values (e.g., `participation=joined,maybe` or `participation=unresponded,maybe`)

### Event Place Field
- Added `place` field to events - optional location/venue for the event
- Available in `POST /api/v1/groups/{groupId}/events` request body
- Available in `PUT /api/v1/events/{id}` request body
- Returned in all event responses (`GET /api/v1/events`, `GET /api/v1/events/{id}`, etc.)

---

## Changelog from 2026-01-25

### New Reports Endpoints
- `POST /api/v1/reports` - Create a content report
- `GET /api/v1/reports/{id}` - Get report details (reporter only)

---

## Changelog from 2026-01-31

### New Poll Endpoints
- `GET /api/v1/polls/{id}` - Get poll detail with all voting information
- `PUT /api/v1/polls/{id}/status` - Change poll status (open/closed)

### Poll Response Enhancements
- Added `is_voted` boolean per option - indicates if current user voted for this option
- Added `voters` array per option - list of users (id, name) who voted for each option
- Added `total_votes` field - total number of votes cast (differs from `total_voters` for multiselect polls)
- Added `can_change_status` boolean - indicates if current user can open/close the poll
- Added `updated_at` timestamp

### Poll Status Change Permissions
Users who can change poll status:
- Poll creator
- Group owner or admin
- Event owner or admin (for event-linked polls)
